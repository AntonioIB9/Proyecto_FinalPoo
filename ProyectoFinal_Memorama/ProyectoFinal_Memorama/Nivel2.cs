...
namespace ProyectoFinal_Memorama
{
    public partial class Nivel2 : Form
    {
        ...
        List<string> imagenes = new List<string>()//Declaracion de una lista para almacenar los caracteres
        {
            ...
            "g","g","h","h","i","i","j","j","k","k","l","l", // Datos que contendra la lista
        ...
        private void BtnIniciar_Click(object sender, EventArgs e)  //Metodo del evento del BotonInicio al dar click
        {
            if (MessageBox.Show("Deseas jugar en modo contra reloj? (120 SEGUNDOS)? ", "salir", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)// Se le mostrara una ventana emergente al usuario de si quiere activar el modo contra reloj
            {
                BtnIniciar.Enabled = false; // Al elegir opcion el mismo boton se congelara para evitar otro ingreso
            }
            else // Condicion Si no
            {
                this.Close(); // Se cerrara dicho formulario
                Nivel2 frm = new Nivel2(); // Creacion del objeto
        ...
        private void TmrPrueba_Tick(object sender, EventArgs e) // Evento generado del TimerPrueba(Modo contra reloj) 
        {
            ...
            if (conteo == 120) // Si conteo es igual al valor 120 
            {
        ...
        private void BtnReiniciar_Click(object sender, EventArgs e) // Evento al dar click al boton Restar
        {
            this.Close(); // Cerrara la forma actual
            Nivel2 N2 = new Nivel2(); // Creación del objeto Nivel1
            N2.Show(); // Mostrara la forma Nivel1
        }

        private void BtnRegresar_Click(object sender, EventArgs e) // Evento al dar click al boton Regresar
        {
            this.Close(); // Cerrar el formulario actual
            Desafio frm = new Desafio(); // Creación del objeto niveles
            frm.Show();// Mostrara la forma niveles
        }
    }
}
